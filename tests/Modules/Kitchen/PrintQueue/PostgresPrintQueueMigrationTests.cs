...
public sealed class PostgresPrintQueueMigrationTests : IAsyncLifetime
{
    ...
    public async Task UpAndDownMigrationsApplyAndRevertCleanly()
    {
        ...
        var upSql = await File.ReadAllTextAsync(Path.Combine(sqlDirectory, "017-print-queue.up.sql"));
        var downSql = await File.ReadAllTextAsync(Path.Combine(sqlDirectory, "017-print-queue.down.sql"));
    ...
    private async Task<bool> CheckTableExistsAsync()
    {
        ...
        cmd.CommandText =
            ...
            WHERE table_schema = 'kitchen' AND table_name = 'print_jobs';
            """;
